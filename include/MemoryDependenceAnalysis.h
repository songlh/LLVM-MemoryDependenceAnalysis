#ifndef _H_SONGLH_MEMORYDEPENDENCEANALYSIS
#define _H_SONGLH_MEMORYDEPENDENCEANALYSIS
...
class MemDepResult {
	...
	typedef llvm::PointerIntPair<llvm::Instruction*, 2, DepType> PairTy;
	PairTy Value;
...
public:
    ...
    /// MemDepResult kinds.
	static MemDepResult getDef(llvm::Instruction *Inst) {
		assert(Inst && "Def requires inst");
		return MemDepResult(PairTy(Inst, Def));
	}

	static MemDepResult getClobber(llvm::Instruction *Inst) {
		assert(Inst && "Clobber requires inst");
		return MemDepResult(PairTy(Inst, Clobber));
	}

	static MemDepResult getNonLocal() {
		return MemDepResult(PairTy(reinterpret_cast<llvm::Instruction*>(NonLocal), Other));
	}

	static MemDepResult getNonFuncLocal() {
		return MemDepResult(PairTy(reinterpret_cast<llvm::Instruction*>(NonFuncLocal), Other));
	}
    
	static MemDepResult getUnknown() {
		return MemDepResult(PairTy(reinterpret_cast<llvm::Instruction*>(Unknown), Other));
	}
	...
	bool isNonLocal() const {
		return Value.getInt() == Other && Value.getPointer() == reinterpret_cast<llvm::Instruction*>(NonLocal);
	}

    /// isNonFuncLocal - Return true if this MemDepResult represents a query
    /// that is transparent to the start of the function.
	bool isNonFuncLocal() const {
		return Value.getInt() == Other && Value.getPointer() == reinterpret_cast<llvm::Instruction*>(NonFuncLocal);
	}

    /// isUnknown - Return true if this MemDepResult represents a query which
    /// cannot and/or will not be computed.
	bool isUnknown() const {
		return Value.getInt() == Other && Value.getPointer() == reinterpret_cast<llvm::Instruction*>(Unknown);
	}
...
private:
	...
	static MemDepResult getDirty(llvm::Instruction *Inst) {
...
class NonLocalDepEntry {
	...
	MemDepResult Result;
public:
	NonLocalDepEntry(llvm::BasicBlock *bb, MemDepResult result)
      : BB(bb), Result(result) {}

    // This is used for searches.
	NonLocalDepEntry(llvm::BasicBlock *bb) : BB(bb) {}
...
class NonLocalDepResult {
	...
	llvm::Value *Address;
public:
	NonLocalDepResult(llvm::BasicBlock *bb, MemDepResult result, llvm::Value *address)
      : Entry(bb, result), Address(address) {}

    // BB is the sort key, it can't be changed.
	llvm::BasicBlock *getBB() const { return Entry.getBB(); }

	void setResult(const MemDepResult &R, llvm::Value *Addr) {
		Entry.setResult(R);
		Address = Addr;
	}
...
class MemoryDependenceAnalysis /*: public FunctionPass*/ {
	...
	typedef llvm::DenseMap<llvm::Instruction*, MemDepResult> LocalDepMapType;
...
private:
	...
	typedef llvm::PointerIntPair<const llvm::Value*, 1, bool> ValueIsLoadPair;
	...
	typedef llvm::PointerIntPair<llvm::BasicBlock*, 1, bool> BBSkipFirstBlockPair;
  ...
  typedef llvm::DenseMap<ValueIsLoadPair, NonLocalPointerInfo> CachedNonLocalPointerInfo;

  CachedNonLocalPointerInfo NonLocalPointerDeps;

    // A map from instructions to their non-local pointer dependencies.
	typedef llvm::DenseMap<llvm::Instruction*,
                     llvm::SmallPtrSet<ValueIsLoadPair, 4> > ReverseNonLocalPtrDepTy;
	...
	typedef llvm::DenseMap<llvm::Instruction*, PerInstNLInfo> NonLocalDepMapType;
	...
	typedef llvm::DenseMap<llvm::Instruction*,
                     llvm::SmallPtrSet<llvm::Instruction*, 4> > ReverseDepMapType;
	...
	llvm::OwningPtr<llvm::PredIteratorCache> PredCache;
public:
  AliasAnalysis *AA;
  llvm::DataLayout *TD;
  llvm::DominatorTree *DT;
	...
	virtual ~MemoryDependenceAnalysis();
	static char ID;
	...
	MemDepResult getDependency(llvm::Instruction *QueryInst);
    ...
    /// that.
	const NonLocalDepInfo &getNonLocalCallDependency(llvm::CallSite QueryCS);
	...
	void getNonLocalPointerDependency(const AliasAnalysis::Location &Loc,
                                      bool isLoad, llvm::BasicBlock *BB,
                                    llvm::SmallVectorImpl<NonLocalDepResult> &Result);

    /// removeInstruction - Remove an instruction from the dependence analysis,
    /// updating the dependence of instructions that previously depended on it.
	void removeInstruction(llvm::Instruction *InstToRemove);
	...
	MemDepResult getPointerDependencyFrom(const AliasAnalysis::Location &Loc,
                                          bool isLoad,
                                          llvm::BasicBlock::iterator ScanIt,
                                          llvm::BasicBlock *BB,
                                          llvm::Instruction *QueryInst = 0);
	...
	static unsigned getLoadLoadClobberFullWidthSize(const llvm::Value *MemLocBase,
                                                    int64_t MemLocOffs,
                                                    unsigned MemLocSize,
...
private:
	MemDepResult getCallSiteDependencyFrom(llvm::CallSite C, bool isReadOnlyCall,
                                           llvm::BasicBlock::iterator ScanIt,
                                           llvm::BasicBlock *BB);
	bool getNonLocalPointerDepFromBB(const llvm::PHITransAddr &Pointer,
